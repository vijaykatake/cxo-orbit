...
const submitInquiry = async (req, res) => {
  try {
    const sponsor = await Sponsor.create({ id: uuidv4(), ...req.body, status: 'inquiry' });

    // Alert admin
    await sendInquiryAlert(
      process.env.SMTP_USER,
      'Sponsor',
      { companyName: sponsor.companyName, contactEmail: sponsor.contactEmail, package: sponsor.package || 'Not specified' }
    ).catch(() => {}); // non-blocking

    res.status(201).json({ success: true, message: 'Thank you! We will get back to you shortly.', data: sponsor });
  } catch (err) {
...
const getAllSponsors = async (req, res) => {
  try {
    const sponsors = await Sponsor.unscoped().findAll({ order: [['createdAt', 'DESC']] });
