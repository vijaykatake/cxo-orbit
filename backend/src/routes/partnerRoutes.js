...
router.post('/inquiry', async (req, res) => {
  try {
    ...
    await sendInquiryAlert(process.env.SMTP_USER, 'Partner', {
      companyName: partner.companyName, contactEmail: partner.contactEmail, type: partner.partnerType || 'N/A',
    }).catch(() => {});
    res.status(201).json({ success: true, message: 'Thank you! We will be in touch.', data: partner });
  } catch (err) {
